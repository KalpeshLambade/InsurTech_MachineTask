...
export const Delete = ({isDelete, taskId}) => {
    ...
    const delectTask = async() => {
        try {
            const response = await axios.delete(`http://localhost:4000/api/v1/delectTask/${deleteTaskID}`);
  ...
  return (
    <>
      <main className=" z-10 absolute w-full h-full flex justify-center items-center backdrop-blur-sm">
        <section className="md:w-[50%] md:h-[30%] flex flex-col justify-between border border-gray-400 rounded-lg">
